...
__global__ inline void gpuConvertUYVY2RGB_kernel(uchar* src, uchar* dst, uint width, uint height)
{
	...
	for(int i=0; i<height; ++i){
		...
		dst[i*width*3+idx*6]   = clamp(1.164f * (y0 - 16)                       + 2.018f * (cb - 128), .0f, 255.f);	
		dst[i*width*3+idx*6+1] = clamp(1.164f * (y0 - 16) - 0.813f * (cr - 128) - 0.391f * (cb - 128), .0f, 255.f);
		dst[i*width*3+idx*6+2] = clamp(1.164f * (y0 - 16) + 1.596f * (cr - 128),                       .0f, 255.f);
					
		dst[i*width*3+idx*6+3] = clamp(1.164f * (y1 - 16)                       + 2.018f * (cb - 128), .0f, 255.f);
		dst[i*width*3+idx*6+4] = clamp(1.164f * (y1 - 16) - 0.813f * (cr - 128) - 0.391f * (cb - 128), .0f, 255.f);
		dst[i*width*3+idx*6+5] = clamp(1.164f * (y1 - 16) + 1.596f * (cr - 128),                       .0f, 255.f);
	}
...
void gpuConvertUYVY2RGB_async(uchar* src, uchar* dst, uint width, uint height, uint numblocks, uint blocksize, cudaStream_t stream)
{
